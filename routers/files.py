...
async def get_upload_url(
    ...
    try:
        # Verify the projects exist and belongs to the user
        ...
        # We are using put_object coz we want to add or update files into s3
        ...
        return {
            ...
            "data": {
                "upload_url": presigned_url,     # This sends it back to the client, so client can use it for uploading
                "s3_key": s3_key,
...
async def confirm_file_upload(
    ...
    try:
        s3_key = confirm_request.get("s3_key")   # s3_key from frontend/client
        ...
        result = supabase.table("project_documents").update({
            "processing_status": "queued"     # Coz now it's ready to be preprocessed
...
async def add_website_url(
    ...
    try:
        ...
        result = supabase.table("project_documents").insert({
            "project_id": project_id,
            'filename': url,
            's3_key': "",    # There will be no s3_key for a website url coz it's not any doc
            'file_size': 0,
            'file_type': 'text/html',
...
async def delete_file(
    ...
    try:
        ...
        s3_key = file_record["s3_key"]    # Coz we need to delete from s3 as well
