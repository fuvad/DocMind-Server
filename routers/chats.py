...
async def get_chat(
    ...
    try:
        ...
        # Add messages to chat object or dict
...
def rrf_rank_and_fuse(search_results_list: List[List[Dict]], weights: List[float] = None, k: int = 60) -> List[Dict]:
    ...
    # Create default weights, If the caller didn't provide weights, give every search method equal weight. Here [0.5, 0.5]
    if weights is None:
        weights = [1.0 / len(search_results_list)] * len(search_results_list)
    
    chunk_scores = {}   # will store the rrf score
    all_chunks = {}     # will store the actual chunk objects
    
    for search_idx, results in enumerate(search_results_list):
        weight = weights[search_idx]    # Get the corresponding weight
        ...
        for rank, chunk in enumerate(results):
            ...
            # Check whether we've already seen this chunk
...
async def send_message(
    ...
    try:
        ...
        # 7. Call LLM & get response (This step is included in the prev step)
